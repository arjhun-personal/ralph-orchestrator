...
fn simulate_events(events: Vec<Event>) -> (Arc<Mutex<ralph_tui::state::TuiState>>, Arc<Mutex<TerminalWidget>>) {
...
fn iteration_changes_on_build_done() {
    ...
    state.lock().unwrap().update(&Event::new("build.done", "Done"));
...
fn terminal_widget_clear_resets_parser() {
    ...
    assert!(total <= 24, "Expected minimal lines after clear, got {}", total);
}
