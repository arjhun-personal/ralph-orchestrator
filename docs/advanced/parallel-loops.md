...
Loops use human-readable names (adjective-noun format) instead of opaque timestamps:
...
Names are generated by extracting keywords from the prompt and combining them with adjective-noun pairs (50 adjectives x 45 nouns = 2,250+ combinations). Configure via:
...
The web dashboard provides a UI for managing parallel loops:

- **Loop status badges** — Visual indicators for each loop state
- **Action buttons** — Context-aware actions based on loop status:
  - `running` → Stop
  - `queued` → Merge Now, Discard
  - `needs-review` → Retry Merge, Discard (with steering input)
  - `crashed` / `orphan` → Discard
- **Steering input** — For `needs-review` loops, provide context to the merge-ralph via a text input. Steering text is written to `.ralph/merge-steering.txt`.
- **Merge-as-task** — Trigger merges via the `loops.triggerMergeTask` tRPC endpoint
